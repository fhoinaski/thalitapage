...
const Header = () => {
  ...
  const renderMenuItems = (isMobile = false) => (
    <ul className={`${isMobile ? 'flex flex-col gap-2.5' : 'flex space-x-6'}`}>
      {menuItems.map((item) => (
        <li key={item} className={isMobile ? 'mb-4 text-white' : ''}>
          <motion.a
            href={`#${item.toLowerCase()}`}
            className={`text-sm uppercase tracking-wider transition-colors bg-white  duration-300 ${
 ...
 <motion.header
      ...
      className="bg-opacity-90 bg-black fixed w-full z-50 shadow-md"
    >
