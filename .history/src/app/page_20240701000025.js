...
import { motion, AnimatePresence, useAnimate, stagger,useInView  } from 'framer-motion';
import { Star, MapPin,Info , Phone, Instagram, Facebook, Menu, X } from 'lucide-react';
      ...
      {menuItems.map((item) => (
        <li key={item} className={`${isMobile ? 'mb-4' : ''}`}>
          <motion.a
            ...
            className={`text-xl uppercase tracking-wider transition-colors duration-300 ${
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 py-4">
        ...
        <h1 className="text-xl md:text-2xl font-serif font-bold" style={{ color: colors['text-white'] }}>
  THALITA CRISTINA <br/>
  <span className='text-sm' style={{ marginTop: '-10px', display: 'block', lineHeight: 'normal' }}>STUDIO & ACADEMY</span>
</h1>
...
const services = [
  {
    ...
    details: "Técnica que proporciona volume e comprimento aos cílios naturais. Manutenção recomendada a cada 15 ou 21 dias."
  },
  {
    name: "Volume Diamante",
    description: "Intensidade e brilho para um olhar cativante.",
    details: "Técnica avançada que cria um efeito de cílios ultra volumosos e brilhantes. Manutenção recomendada a cada 15 ou 21 dias."
  },
  {
    name: "Volume Brown",
    description: "Extensões de cílios em tom marrom para um efeito natural.",
    details: "Perfeito para quem busca um look mais suave e natural. Utiliza fios marrom. Manutenção recomendada a cada 15 ou 21 dias."
  },
  {
    name: "Efeito Sirena",
    description: "Extensão no canto externo para um olhar felino e sedutor.",
    details: "Técnica que alonga os cílios no canto externo dos olhos, criando um efeito de olhar levantado. Manutenção recomendada até 21 dias."
  },
  {
    name: "Efeito Molhado",
    description: "Aparência brilhante e sedutora para os cílios.",
    details: "Cria um visual de cílios sempre brilhantes e definidos, como se estivessem molhados. Manutenção recomendada a cada 15 ou 21 dias."
  },
  {
    name: "Fox Eyes",
    description: "O queridinho das celebridades para um olhar alongado.",
    details: "Técnica que eleva o olhar, criando um efeito de olhos de raposa. Muito popular entre celebridades. Manutenção recomendada a cada 15 ou 21 dias."
  },
  {
    name: "Design de Sobrancelhas com Henna",
    description: "Design personalizado com aplicação de henna para um resultado duradouro.",
    details: "Inclui higienização, esfoliação, tonificação, design personalizado, aplicação de henna, finalização com gel calmante e pasta mágica."
  },
  {
    name: "Design de Sobrancelhas",
    description: "Design personalizado para realçar o seu olhar.",
    details: "Inclui higienização, esfoliação, tonificação, design personalizado, finalização com gel calmante e pasta mágica."
  }
];
      ...
      >
        ...
        >
          <div className="flex justify-between items-start mb-4">
            <h3 className="text-xl font-semibold" style={{ color: colors.secondary }}>{service.name}</h3>
            <button onClick={onClose}>
              <X size={24} color={colors.accent} />
            </button>
          </div>
          <p className="text-sm mb-6" style={{ color: colors.text }}>{service.details}</p>
        </motion.div>
      </motion.div>
    )}
  </AnimatePresence>
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <ServiceCard key={index} service={service} onInfoClick={handleInfoClick} />
          ))}
        </div>
      </div>
      <Modal 
