...
import { Star, MapPin, Phone, Instagram, Menu, X } from 'lucide-react';
...
const MenuToggle = ({ toggle, isOpen }) => (
  <button
    onClick={toggle}
    className={`w-12 h-12 rounded-full p-3 focus:outline-none z-50 transition-all duration-300 ${
...
const Header = () => {
  ...
  useEffect(() => {
    animate([
      ...
      [
        ...
        { duration: 0.3, delay: isOpen ? stagger(0.05) : stagger(0.05, { from: "last" }) }
      ]
    ]);
  }, [isOpen, animate]);
  ...
  return (
    <motion.header
      ...
      className="bg-opacity-90 bg-white opacity-90 fixed w-full z-50 shadow-md"
    >
      <div className="container mx-auto px-4 py-4 flex justify-between items-center">
        <h1 className="text-2xl md:text-3xl font-serif font-bold" style={{ color: colors.terciary }}>
          Thalita Cristina
        </h1>
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            />
            <motion.nav
              className="fixed top-0 right-0 bottom-0 w-64 bg-[#CE9D81] pt-24 z-50"
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
              ...
              <ul className="flex flex-col gap-2.5 p-4">
                {menuItems.map((item) => (
                  ...
                  >
                    <a
                      href={`#${item.toLowerCase()}`}
                      className="text-lg uppercase tracking-wider transition-colors duration-300 block font-bold text-4xl p-10"
      ...
      <div className={`md:hidden fixed top-2 right-4 ${isOpen ? 'hidden' : ''} transition-all duration-300`}>
        <MenuToggle toggle={toggleMenu} isOpen={isOpen} />
      </div>
    </motion.header>
  );
